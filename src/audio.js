...
export const Snd = {
  ...
  // 鐘は倍音が整数比になっていない(だから「鐘の音」に聞こえる)。
  //   ハム音・プライム・短3度・5度・ノミナル…を別々の減衰で重ねる。
  //   idx は色の周回位置。上がっていくほど高くなるので、目を離していても
  //   今どの色にいるかが耳で分かる。
  bell(idx = 0) {
    if (!this.ctx || this.muted) return;
    const at = this.ctx.currentTime;
    const base = 660 * Math.pow(2, (idx % 6) / 12);
    const PARTIALS = [
      [0.5, 0.055, 0.9], [1.0, 0.075, 0.55], [1.183, 0.05, 0.42],
      [1.506, 0.038, 0.34], [2.0, 0.03, 0.26], [2.66, 0.018, 0.2], [3.01, 0.012, 0.16],
    ];
    for (const [ratio, vol, dur] of PARTIALS) {
      this._tone(base * ratio, 'sine', dur, vol, at, null, 2);
    }
    this._noise(0.012, 0.05, at);            // 撞木が当たる瞬間
  },
