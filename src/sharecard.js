...
export function buildCanvas(d) {
  ...
  drawText(c, 'EMOJI BLASTERS', cx, 100, displayFont(56, 800), '#ffd23f',
...
export function summaryText(d) {
  const ja = getLang() === 'ja';
  const parts = [];
  parts.push(ja ? `EMOJI BLASTERS で ${d.score} 点!` : `Scored ${d.score} in EMOJI BLASTERS!`);
...
export async function shareCard(d) {
  ...
  if (blob && navigator.canShare) {
    try {
      const file = new File([blob], 'emoji-blasters-score.png', { type: 'image/png' });
      if (navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], text: txt, title: 'EMOJI BLASTERS' });
        return 'shared';
      }
  ...
  try {
    ...
    a.href = URL.createObjectURL(blob); a.download = 'emoji-blasters-score.png';
