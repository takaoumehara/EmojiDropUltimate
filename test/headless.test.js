...
test('同じ種は同じ結果になる(サーバーとクライアントで再現できる)', async () => {
  // ⚠ ここが「同一プロセスで2回 boot」しても通るのは、**ゲーム内1分しか
  //   回していないから**。モジュールはプロセス内でキャッシュされるので、
  //   2回目の boot() は学習するボス・ディレクター・札の履歴を引き継ぐ。
  //   1面クリアの長さまで伸ばすと、同じ種でも結果が変わる(実測済み。
  //   詳細は headless.js のヘッダ)。長い実行の決定性は、プロセスを
  //   分けて確かめること —— test/sim.test.js がワーカー経由でやっている。
  const run = async () => {
